...
export class SystemPromptLoader {
   ...
   */
  private findSystemPromptPath(): string {
    const possiblePaths = [
   ...
   */
  async load(): Promise<SystemPromptResult> {
   ...
   */
  private loadFromFileSync(filePath: string): string {
    try {
  ...
  validate(content: string): SystemPromptValidation {
    ...
    if (approximateTokens > 50000) {
      issues.push(
        `System prompt is very large (approximately ${approximateTokens} tokens)`
      );
    }
 ...
 */
export function formatForApi(content: string): string {
 ...
 */
let defaultLoader: SystemPromptLoader | null = null;

/**
 * Gets or creates the default SystemPromptLoader instance.
 */
export function getDefaultLoader(): SystemPromptLoader {
  if (!defaultLoader) {
